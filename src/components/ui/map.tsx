...
/**
 * mapcn — map primitives, vendored.
 *
 * Copied from the mapcn registry (https://mapcn.dev, `AnmolSaini16/mapcn`,
 * `src/registry/map.tsx`) rather than installed: mapcn follows the shadcn
 * model, where the component source is yours to own and edit. This project has
 * no shadcn setup — no `components.json`, no `components/ui` registry — so the
 * file is placed here by hand and its imports point at this repo's own `cn`.
 *
 * Everything below is upstream except for three deliberate changes, all of
 * them marked `// prime:` in place:
 *
 *   1. **Tiles.** The default basemaps are OpenFreeMap, not CARTO. OpenFreeMap
 *      needs no API key and no account, which is the reason this replaced the
 *      Google Maps build — the Google map stopped rendering once the demo key
 *      hit its daily quota. `src/lib/siteMapStyle.ts` repaints the OpenFreeMap
 *      style in the site's own tokens; components call `paintSiteBasemap()`.
 *
 *   2. **The worker is served from this origin.** Upstream points MapLibre's
 *      worker at unpkg. `scripts/sync-maplibre-worker.mjs` copies it into
 *      `public/vendor/maplibre/` on `postinstall` instead, so no third-party
 *      CDN sits in the render path.
 *
 *   3. **Theme tokens.** Upstream is written against shadcn's semantic classes
 *      (`bg-popover`, `border-border`, …), which do not exist in this project's
 *      Tailwind theme and would have rendered as transparent boxes. They are
 *      mapped to the site tokens — paper/ink/brass/line — and the rounded
 *      corners are squared off, because every other panel on this site is.
 *
 * Re-pulling a newer mapcn means re-applying those three. Nothing else here
 * should be edited to suit a single page; wrap it in a component instead.
 */
...
// prime: self-hosted worker. MapLibre v6 derives this from its own
// `import.meta.url`, which stops being a resolvable http URL once Next bundles
// the library, so it comes back empty and the map never paints. Upstream falls
// back to unpkg; `scripts/sync-maplibre-worker.mjs` puts the exact installed
// version under `public/vendor/maplibre/` on postinstall instead.
if (typeof window !== 'undefined' && !MapLibreGL.getWorkerUrl()) {
  MapLibreGL.setWorkerUrl('/vendor/maplibre/maplibre-gl-worker.mjs')
}

// prime: OpenFreeMap instead of CARTO — no key, no account, no quota. The
// light style is the one this site uses; `dark` is kept so the upstream
// theme-switching code path stays intact.
const defaultStyles = {
...
function DefaultLoader() {
  return (
    <div className="bg-paper/60 absolute inset-0 z-10 flex items-center justify-center backdrop-blur-xs">
      <div className="flex gap-1">
        <span className="bg-brass/60 size-1.5 animate-pulse rounded-full" />
        <span className="bg-brass/60 size-1.5 animate-pulse rounded-full [animation-delay:150ms]" />
        <span className="bg-brass/60 size-1.5 animate-pulse rounded-full [animation-delay:300ms]" />
      </div>
    </div>
  )
}
...
function PopupCloseButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="focus-visible:ring-brass hover:bg-paper-2 text-ink absolute top-1 right-1 z-10 inline-flex size-5 cursor-pointer items-center justify-center transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-inset"
    >
...
function MarkerPopup({
  ...
  return createPortal(
    <div
      className={cn(
        'bg-white text-ink relative max-w-62 border p-3 shadow-md',
...
function MarkerTooltip({ children, className, ...popupOptions }: MarkerTooltipProps) {
  ...
  return createPortal(
    <div
      className={cn(
        'bg-ink text-paper pointer-events-none px-2 py-1 text-xs text-balance shadow-md',
        'animate-in fade-in-0 zoom-in-95 duration-200 ease-out',
        className,
      )}
    >
  ...
  return (
    <div
      className={cn(
        ...
        'text-ink text-[10px] font-medium',
...
function ControlGroup({ children }: { children: React.ReactNode }) {
  return (
    <div className="border-line bg-paper [&>button:not(:last-child)]:border-line flex flex-col overflow-hidden border shadow-sm [&>button:not(:last-child)]:border-b">
      {children}
    </div>
  )
}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        'hover:bg-white hover:text-brass-deep',
        'focus-visible:ring-brass focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-inset',
