...
export function LocationMap({
  ...
  const center: [number, number] = placeable.length
    ? [placeable[0].lng, placeable[0].lat]
    : [0, 0]

  const initialZoom = placeable.length > 1 ? SINGLE_PLACE_ZOOM : (zoom ?? SINGLE_PLACE_ZOOM)

  const fitAll = useCallback(() => {
    const map = mapRef.current
    if (!map || placeable.length < 2) return
    const bounds = new LngLatBounds()
    placeable.forEach(({ lat, lng }) => bounds.extend([lng, lat]))
      ...
      >
        {inView ? (
          <MapCanvas
            ...
            // against OpenFreeMap's positron layer names, so letting mapcn
            // follow the OS dark-mode preference would paint the `dark` style
            // instead and leave the map in raw dark grey.
            theme="light"
            center={center}
            zoom={initialZoom}
            minZoom={MIN_ZOOM}
              ...
              >
                <MarkerContent>
                  <span className="relative block h-9 w-[26px]">
                    {/* Ground shadow: the pin stands on the map rather than
                        floating over it. */}
                    <span
                      aria-hidden
