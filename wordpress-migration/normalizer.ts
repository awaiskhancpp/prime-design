...
const mediaFromValue = (value: unknown): NormalizedImage[] => {
  ...
  const walk = (value: unknown) => {
    ...
    if (
      ...
      (typeof object.full === 'string' ||
        typeof object.url === 'string' ||
        typeof object.filename === 'string')
    ) {
      result.push({
        sourceId: object.id,
        filename: typeof object.filename === 'string' ? object.filename : undefined,
        url:
          typeof object.full === 'string'
            ? object.full
            : typeof object.url === 'string'
...
const blockedContentWrappers = new Set([
  'tabs-nested',
  'xproaccordion',
  'slider-nested',
  'carousel',
...
const hiddenClass = (node: BricksTreeNode) => {
  ...
  return hidden &&
    ...
    typeof (hidden as Record<string, unknown>)._cssClasses === 'string'
...
function faqFromNode(section: BricksTreeNode): NormalizedFaqCategory[] {
  ...
  const titles = nodes
    .filter((node) => hiddenClass(node).includes('tab-title'))
    .map((node) => ({
      sourceId: node.id,
      title: cleanText(
        (nodes.find((child) => child.parent === node.id && child.name === 'text-basic')?.settings
          .text as string) ||
          textValue(node) ||
          '',
      ),
  ...
  const queries = nodes
    .filter((node) => node.settings.query)
    .map((node) => node.settings.query as Record<string, unknown>)
  if (titles.length === 0) {
    // Some pages implement their FAQ section as a bare query loop (post_type:
    // faq, filtered by faq-category taxonomy) with no tabs/accordion widget
    // at all — just a text-basic node whose text is the unresolved
    // "{post_content}" template. There's nothing to build per-tab titles
    // from, but the query itself is real and resolvable.
    const faqQuery = queries.find((query) => JSON.stringify(query.post_type || '').includes('faq'))
    if (faqQuery) {
      return [{ sourceId: section.id, title: '', query: faqQuery, questions: [] }]
    }
    return []
  }
...
function testimonialsFromNode(section: BricksTreeNode): NormalizedTestimonial[] {
  ...
  const titles = nodes
    ...
    .map((node) => {
      const child = nodes.find(
        (candidate) => candidate.parent === node.id && candidate.name === 'text-basic',
      )
      return {
        sourceId: node.id,
        title: cleanText(typeof child?.settings.text === 'string' ? child.settings.text : ''),
      }
    })
    .filter((item) => item.title)
  if (titles.length === 0) {
  ...
  const shortcodes = nodes
    ...
    .map((node) =>
      typeof node.settings.shortcode === 'string' ? node.settings.shortcode : undefined,
    )
  ...
  return titles.map((title, index) => {
    ...
    return {
      sourceId: title.sourceId,
      provider: title.title.replace(/\s+rating$/i, ''),
      title: title.title,
      shortcode,
      collectionId,
    }
...
function galleryFromNode(section: BricksTreeNode): NormalizedGalleryItem[] {
  const nodes = descendants(section)
  const items: NormalizedGalleryItem[] = nodes
    .filter((node) => node.name === 'image')
    .flatMap((node) => {
  ...
  for (const node of nodes.filter(
    (item) => item.name === 'image-gallery' || item.name === 'happyfiles-gallery',
  )) {
    const settings = node.settings
    const dynamicSource =
      typeof (settings.items as Record<string, unknown> | undefined)?.useDynamicData === 'string'
        ? (settings.items as Record<string, string>).useDynamicData
        : undefined
    const galleryId =
      typeof (settings.ids as Record<string, unknown> | undefined)?.['0'] === 'string'
...
function mediaItemsFromNode(section: BricksTreeNode): NormalizedGalleryItem[] {
  ...
  for (const node of descendants(section).filter((item) => item.name === 'video')) {
    const url = Object.values(node.settings).find(
      (value): value is string =>
        typeof value === 'string' && /(?:youtube|youtu\.be|vimeo|\.mp4|\.webm|\.mov)/i.test(value),
    )
    if (url)
      items.push({ sourceId: node.id, mediaType: 'video', sourceUrl: url, settings: node.settings })
  }
  return items
}
...
function integrationsFromNode(section: BricksTreeNode): NormalizedIntegration[] {
  return descendants(section)
    .filter((node) => node.name === 'shortcode' || node.name === 'xfluentform')
    .flatMap((node) => {
      const shortcode =
        typeof node.settings.shortcode === 'string' ? node.settings.shortcode : undefined
...
function listItemsFromNode(node: BricksTreeNode) {
  ...
  return Object.values(items as Record<string, unknown>)
    .map((item) =>
      item && typeof item === 'object' ? (item as Record<string, unknown>).title : undefined,
    )
...
function repairCategories(section: BricksTreeNode): Array<{
  ...
  const groups =
    container?.children.filter((node) => {
      ...
      return (
        node.name === 'block' &&
        nodes.some((child) => child.name === 'text-basic') &&
        nodes.some((child) => child.name === 'image')
      )
    }) || []
  const structured = groups
    .map((group) => {
      ...
      const description = textNodes
        .slice(1)
        .map((node) => cleanText(textValue(node) || ''))
  ...
  const html = descendants(section)
    .filter((node) => node.name === 'text' || node.name === 'text-basic' || node.name === 'code')
    .map((node) =>
      node.name === 'code' && typeof node.settings.code === 'string'
        ? node.settings.code
        : textValue(node),
    )
    .filter((value): value is string => Boolean(value))
  ...
  const categories = matches
    .map((match, index) => {
      ...
      const description = cleanText(
        segment.replace(match[0], '').replace(/<ul>[\s\S]*?<\/ul>/i, ''),
      )
      const features = [...segment.matchAll(/<li[^>]*>([\s\S]*?)<\/li>/gi)]
...
function classify(
  ...
  const text = descendants(section)
    .map(textValue)
    .filter((value): value is string => Boolean(value))
    .join(' ')
    .replace(/<[^>]+>/g, ' ')
  ...
  const hiddenByCss =
    typeof section.settings._cssCustom === 'string' &&
    new RegExp(`#brxe-${section.id}\\s*\\{[^}]*display\\s*:\\s*none`, 'i').test(
      section.settings._cssCustom,
    )
  if (section.settings._visibility === 'hidden' || section.settings._opacity === '0' || hiddenByCss)
    return 'utility'
  if (
    section.settings._position === 'absolute' ||
    section.settings._position === 'fixed' ||
  ...
  if (
    ...
    descendants(section).some((node) => {
      const query = node.settings.query
      return (
        query &&
        typeof query === 'object' &&
        JSON.stringify((query as Record<string, unknown>).post_type || '').includes('faq')
      )
    })
  )
    return 'faq'
  ...
  if (
    ...
    heading.includes('protecting') ||
    heading.includes('elevating your outdoor living')
  )
    return 'image-text'
  ...
  if (names.some((name) => name === 'image') && names.some((name) => /heading|text/i.test(name)))
    return 'image-text'
...
function semanticClassification(
  ...
  if (type === 'utility' || sticky || names.some((name) => utilityNames.has(name))) {
    return {
      ...
      reason:
        'This root section is a sticky/navigation utility structure embedded in the Bricks export.',
      required: 'Preserve only if the landing-page shell explicitly owns this utility.',
    }
  }
  if (type === 'booking') {
    return {
      ...
      reason:
        'The section contains a shortcode-backed booking integration whose provider fields require runtime verification.',
      required: 'Verify the booking provider and preserve its configuration',
    }
  }
  if (type === 'unsupported') {
    return {
      classification: 'missing-schema' as const,
      reason: `No section normalizer matched Bricks elements: ${[...new Set(names)].join(', ')}.`,
      required: 'A dedicated Payload block and renderer',
    }
  }
  if (
    names.some((name) =>
    ...
    return {
      classification: 'partial' as const,
      reason: `The section is recognized, but contains third-party or nested Bricks elements: ${[...new Set(names.filter((name) => !supportedNames.has(name) || ['tabs-nested', 'xproaccordion', 'slider-nested', 'carousel', 'xfluentform', 'shortcode'].includes(name)))].join(', ')}.`,
      required: 'Verify nested data and renderer behavior before import',
    }
  }
...
export function normalizeBricksPage(
  page: WordPressPage,
  ...
  roots
    ...
    .forEach((section, index) => {
      ...
      if (type === 'booking' || type === 'contact-form' || type === 'form')
        nestedData.integrations = integrationsFromNode(section)
      ...
      sections.push({
        ...
        data: {
          ...
          nestedElementIds: descendants(section).map((node) => node.id),
          sourceTree: section,
        },
        images,
        videos,
        classification:
          unsupportedElements.length > 0 && semantic.classification === 'supported'
            ? 'partial'
